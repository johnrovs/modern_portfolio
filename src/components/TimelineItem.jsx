...
export default function TimelineItem({ item, index, isLast }) {
  ...
  return (
    <div className="relative grid md:grid-cols-2 gap-6 md:gap-12">
      ...
      <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-px bg-overlay/10">
      ...
      >
        ...
        <h3 className="font-heading font-bold text-lg sm:text-xl text-textPrimary">{item.position}</h3>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {item.tech.map((t) => (
            <span
              key={t}
              className="text-xs font-mono px-2.5 py-1 rounded-full bg-overlay/5 border border-overlay/10 text-textSecondary"
            >
              {t}
            </span>
          ))}
