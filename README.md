...
3. **Create a template** → *Email Templates* → *Create New Template*. Set it up like this:
   ...
   - **Subject**: `{{subject}}`
   - **Content**: switch the editor to **Code / HTML view** and paste in `emailjs-contact-template.html` (included in this project root) — a branded template matching the portfolio's look, with the sender's name, email, timestamp, and message laid out in a styled card, plus a one-click "Reply" button.
